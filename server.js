...
const { prefix, token, token2, dbltoken } = require('./config.json');
const fs = require('fs');
...
const client = new OredonClient({disableEveryone: true});
...
client.rbl = rbl;
client.db = new db.table("database")
client.guild = new db.table("guilddatabase")
client.afk = new db.table("AFKDatabase")
...
client.translate = async(text, message) => {
  let lang = await client.guild.has(`guild.${message.guild.id}.language`) ? await client.guild.get(`guild.${message.guild.id}.language`) : 'en';
...
/* The end codes of command handler */
...
client.on('guildMemberAdd', async (member) => {
  ...
  let autorolesStatus = client.guild.get(`guild.${member.guild.id}.autoroles.type`)
  let welcomeStatus = client.guild.get(`guild.${member.guild.id}.welcome.type`)
  let welcomeChannel = client.guild.get(`guild.${member.guild.id}.welcome.channelID`)
  let welcomeMessage = client.guild.get(`guild.${member.guild.id}.welcome.welcomeMessage`)
  if (!welcomeChannel && welcomeChannel == null) return;
  if (!welcomeStatus && welcomeStatus == false) return;
  if (!welcomeMessage && welcomeMessage == null || undefined) welcomeMessage = "Welcome to {server}'s server {user} You're the {membercount} member!"
  //welcomeMessage.toLowerCase()
  welcomeMessage = welcomeMessage.replace(/{user}/gi, member.toString())
  welcomeMessage = welcomeMessage.replace(/{server}/gi, member.guild.name.toString())
  welcomeMessage = welcomeMessage.replace(/{membercount}/gi, member.guild.memberCount.toString())
  welcomeMessage = welcomeMessage.replace(/{usertag}/gi, member.user.tag.toString())
  welcomeMessage = welcomeMessage.replace(/{username}/gi, member.user.username.toString())
...
client.on('guildMemberRemove', async (member) => {
  ...
  let goodbyeChannel = client.guild.get(`guild.${member.guild.id}.goodbye.channelID`)
  let goodbyeMessage = client.guild.get(`guild.${member.guild.id}.goodbye.goodbyeMessage`)
  if (!goodbyeChannel && goodbyeChannel == null) return;
  if (!goodbyeStatus && goodbyeStatus == false) return;
  if (!goodbyeMessage && goodbyeMessage == null || undefined) goodbyeMessage = "Goodbye from {server} {user} I hope you come back!"
  //welcomeMessage.toLowerCase()
  goodbyeMessage = goodbyeMessage.replace(/{user}/gi, member.toString())
  goodbyeMessage = goodbyeMessage.replace(/{server}/gi, member.guild.name.toString())
  goodbyeMessage = goodbyeMessage.replace(/{membercount}/gi, member.guild.memberCount.toString())
  goodbyeMessage = goodbyeMessage.replace(/{usertag}/gi, member.user.tag.toString())
  goodbyeMessage = goodbyeMessage.replace(/{username}/gi, member.user.username.toString())
...
client.on('ready', async() => {
  ...
  rbl.updateStats(client.guilds.cache.size, client.guilds.cache.reduce((a, b) => a + b.memberCount, 0))
  setInterval(() => client.user.setActivity(`o!help | ${activities[i++ % activities.length]}`, {type: "WATCHING"}), 10000)
});
...
client.on('message', async(msg) => {
  ...
  if (msg.content.startsWith(`<@!${client.user.id}>`) || msg.content.startsWith(client.user)) return msg.inlineReply(":wave: My default prefix is \`o!\` type \`o!help\` for get commands help.");
  ...
  if (member) {
    ...
    if (afk) {
      ...
      return msg.reply(`${member} is currently afk (${timeAgo}) \nReason: \`${reason}\``)
    }
  }
  ...
  if (cmd == "rbl") {
    ...
    if (user.bot == false) return msg.channel.send('User must bot!')
      rbl.getBot(user.id).then(x => {
        if (x == null || undefined) return msg.channel.send("This bot is not registered in **Renzyh Bots List**")
        let yes = []
  ...
  if (!cmd) return;
  if (!command) return msg.inlineReply("Command not found! type \`o!help\` for get a commands help.").then(x => x.delete({timeout: 5000}));
  ...
  if (timestamps.has(msg.author.id)) {
    ...
    if (now < expirationTime) {
      ...
      return msg.channel.send(`⏱ | Please wait, **${timeLeft.toFixed(1)}** more second(s) before reusing the \`${command.name}\` command.`).then(c => c.delete({ timeout: 5000 }));
    };
  }
...
//      if (err) console.log(err);
      /* Apa bila player tsb tidak terblacklist */
      exports.msg = msg;
      if (!blacklist) {
        /* Maka sistem akan menjalankan perintah dari player tsb */
        command.run(client, msg, args);
      } else {
      /* Apa bila player tsb terblacklist maka tidak bisa menjalankan perintah tersebut dan akan dikirimkan sebuah pesan melalui dm */
          msg.author.send(`<:ModerationHammer:597740719570026497> | Hey, ${msg.author} you has been blacklisted from this bot beacuse violating a rules of the bot!`).catch(err => {
          return msg.channel.send(`I thinks you disable your DMs, so i will send this message to this channel!`).then(x => {
            x.edit(`<:ModerationHammer:597740719570026497> | Hey, ${msg.author} you has been blacklisted from this bot beacuse violating a rules of the bot!`)
          })
        })
      }
//      })
...
async function handleVideo(video, message, voiceChannel, playlist = false) {
    ...
    .addField("<:timer:772330397622075412> Duration:", `${require('./util.js').timeString(song.durationmm)}`, true)
    .addField('<:youtubers:529206401327955998> Uploaded by:', `**[${song.uploaded}](${song.channel})**`, true)
    .addField('<:myaudio:772330443005231154> Voice Channel:', song.voicechan, true)
    .addField('<:profileblue:772330368392888361> Requested By:', song.authors.tag, true)
